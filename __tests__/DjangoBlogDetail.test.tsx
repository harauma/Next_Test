...
describe('DjangoBlogDetailPage Test Cases', () => {
  it('Should render detailed content of ID 1', async () => {
    const { page } = await getPage({
      route: '/django-posts/1',
    })
    render(page)
  ...
  it('Should route back to django-blog-page from detail page', async () => {
    const { page } = await getPage({
      route: '/django-posts/2/',
    })
    render(page)
    await screen.findByText('title2')
    userEvent.click(screen.getByTestId('back-django-blog'))
